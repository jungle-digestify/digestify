...
export default function VideoView(all: any) {
    ...
    if(!showVideo){
        ...
        return (
        <div className='h-full w-full flex flex-col gap-5 overflow-y-auto' style={{ border: '10px solid rgba(0,0,0,0)' , boxSizing:'border-box'}}>
            ...
            <div className='timeLine gap-4 px-5'>
                ...
                timeLineArr.map((time: any, index:number)=>(
                    <div key={index} ><button className='showTimeLine' onClick={()=> 
                        timeLineBtn(timeLineArr[index]) 
                        
                }>{timeLineArr[index]?.toString()}</button></div>
                ))
